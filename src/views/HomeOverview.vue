...
//  v-if="authStore.hasPermission('read_events')"
</script>
...
<template>
	...
	<div class="event-host-info">
		<h2>{{ eventHostName }}</h2>
		<div v-if="timerLogo" class="timer-logo">
			<img :src="timerLogo" alt="" />
		</div>
		<div v-html="timerInfo" class="timer-info"></div>
	</div>
</template>
...
.event-detail-panel {
	...
	border-color: light-dark(
		oklch(0.9 calc(var(--chr_0) / 3) var(--hue_0)),
		oklch(var(--lum_0_high) var(--chr_0) var(--hue_0))
	);
	background-color: light-dark(
		oklch(var(--lum_0_high) var(--chr_0) var(--hue_0) / 5%),
		oklch(var(--lum_0_low) var(--chr_0) var(--hue_0) / 15%)
	);
	padding: 1rem;
	margin: 1rem 0;
...
.timer-logo {
	...
	border-block-end: solid thin currentColor;
}
